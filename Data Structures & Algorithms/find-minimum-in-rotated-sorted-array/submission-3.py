...
class Solution:
    def findMin(self, nums: List[int]) -> int:
        # Search between the current left and right boundaries.
        # The minimum is guaranteed to be somewhere in this range.
        left, right = 0, len(nums) - 1

        # Continue searching while the range contains more than one element.
        while left < right:
            # Find the middle index.
            mid = left + (right - left) // 2

            # If nums[mid] is greater than the rightmost value,
            # then the rotation point—and therefore the minimum—must
            # be to the right of mid.
            if nums[mid] > nums[right]:
                left = mid + 1

            # Otherwise, the minimum is at mid or somewhere to its left.
            # Keep mid in the search range by setting right = mid.
            else:
                right = mid

        # When left == right, both pointers refer to the minimum element.
        return nums[left]
